...
- (void)UMessageDidReceiveRemoteNotification:(NSDictionary *)userInfo window:(UIWindow *)window {
    ...
    self.window = window;
    //定制自定的的弹出框
    if([UIApplication sharedApplication].applicationState == UIApplicationStateActive) {//前台运行
        NSDictionary *aps = userInfo[@"aps"];
        NSString *message = aps[@"alert"];
        NSString *title = userInfo[@"type"];
...
- (void)showMessage {
    ...
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"userInfoNotification" object:self userInfo:@{@"userinfo":self.userInfo}];
        return;
    }
    // 否则，跳转到我的消息
    [MessageViewController showMessageViewController:baseVC show:^(BOOL show) {
